...
Alright, so far you have learned about property binding, and it's variations like class binding, style binding, and
attribute binding, to add something in the DOM to display data in Angular we also have event binding which
we use to handle events raised from the DOM, like keystrokes, mouse movements, clicks and so on. So here we have a simple button,
and want to handle the click event of this button. So, Instead of square brackets we use parenthesis, and here we add the name of the event like click,
...
For example with mouse movements, they even object will tell us the x and y position, if you want to get
access to that event object, we need to add that as a parameter here. So '$event' and then when calling this method we also pass '$event'.
